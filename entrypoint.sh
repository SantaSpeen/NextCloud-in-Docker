...
# Костыли, что бы у докера был доступ к 1000 UID
if [ $(getent group gmax) ]; then
  echo "Group 'gmax' exists."
else
  echo "Group 'gmax' does not exist. Creating group with GID 1000..."
  apk add shadow
  /usr/sbin/groupadd -g 1000 gmax
  usermod -a -G gmax www-data
